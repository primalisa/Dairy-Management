    ...
    <body style="background-color:#8cd5ff; ;" >
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
            <table border="1" cellspacing="5" cellpadding="5" align="center">
                <tr align="center"><th>DATE</th><th>TIME</th><th>Farmer ID</th><th>QTY/LTR</th><th>FAT</th></tr>
                <tr align="center">
                    <td><input type="text" name="date" id="date"/></td>
                    <td><select name="time" id="time"><option>Morning</option><option>Evening</option></select></td>
                    <td><select name="ssn" id="ssn">
                        <?php
                        $result1 = mysqli_query($mysql, "SELECT * FROM farmer");
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $total =0;
            $rate=0;
            $result2 = mysqli_query($mysql, "SELECT * FROM farmer WHERE ssn ='$ssn'");
